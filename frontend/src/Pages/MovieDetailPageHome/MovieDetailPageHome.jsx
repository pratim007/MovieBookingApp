...
export default function MovieDetailPage() {
  ...
  const showtimeDays = useMemo(() => {
    ...
    const candidates = [
      ...
      ...(Array.isArray(movie.slots) ? movie.slots : []),
    ];
    ...
    const result = Object.keys(slotsByDate)
      .sort()
      .map((key) => {
    ...
    console.debug("Computed showtimeDays:", result, "rawSlots:", candidates, "movie:", movie);
    return result;
  }, [movie]);
              ...
              <div className={movieDetailHStyles.daySelection}>
                {showtimeDays.length ? (
                  ...
                  <div className="text-gray-400 text-sm px-2">
                    <div>No showtime dates available</div>
                    <details className="mt-2 text-xs text-left p-2 bg-black/40 rounded">
                      <summary className="cursor-pointer">Debug: show raw slot data</summary>
                      <pre className="whitespace-pre-wrap break-all mt-2 text-[11px]">{JSON.stringify({
                        slots: movie?.slots || null,
                        normalized: normalizeSlotSource(movie),
                        auditorium: movie?.auditorium || null,
                      }, null, 2)}</pre>
                    </details>
                  </div>
                )}
