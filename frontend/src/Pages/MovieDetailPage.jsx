...
function to24Hour(timeStr = "00:00", ampm = "") {
  ...
  const explicitAmpm = String(ampm || "").trim().toUpperCase();

  const ampmMatch = raw.match(/^(\d{1,2})(?::(\d{1,2}))?\s*(AM|PM)$/i);
  if (ampmMatch) {
    let h = Number(ampmMatch[1]);
    let m = String(ampmMatch[2] || "00").padStart(2, "0");
    const meridiem = ampmMatch[3].toUpperCase();
    if (meridiem === "AM" && h === 12) h = 0;
    if (meridiem === "PM" && h !== 12) h = h + 12;
    return `${String(h).padStart(2, "0")}:${m}`;
  }

  const compact = raw.replace(/[^0-9:]/g, "");
  const [hRaw = "0", mRaw = "00"] = compact.split(":");
  let h = Number(hRaw || 0);
  const m = String(Number(mRaw || 0) || 0).padStart(2, "0");

  if (explicitAmpm) {
    if (explicitAmpm === "AM") {
      if (h === 12) h = 0;
    } else if (explicitAmpm === "PM") {
      if (h !== 12) h = h + 12;
    }
  }
...
function slotToISO(slot) {
  ...
  if (typeof slot === "object") {
    if (slot.date && (slot.time || slot.datetime || slot.iso || slot.slotTime || slot.startTime || slot.timeText)) {
      const date = slot.date;
      const timeStr = slot.time || slot.datetime || slot.iso || slot.slotTime || slot.startTime || slot.timeText || "00:00";
      const ampm = slot.ampm || slot.amp || "";
      const hhmm = to24Hour(timeStr, ampm);
      return `${date}T${hhmm}:00+05:30`;
    }
    if (slot.datetime || slot.iso || slot.dateTime) return slot.datetime || slot.iso || slot.dateTime;
    if (slot.time && !slot.date) return slot.time;
  }
  return null;
}
  ...
  for (const source of sources) {
    ...
    if (Array.isArray(source)) {
      ...
      try {
        ...
        if (Array.isArray(parsed)) collect.push(...parsed);
        else if (parsed) collect.push(parsed);
      } catch {
        collect.push(source);
      }
...
export default function MovieDetailPage() {
    ...
    (async function fetchMovie() {
      ...
      try {
        ...
        if (!data || data.success === false) {
          ...
          console.debug("Fetched movie (for debugging):", item);
          setMovie(item || null);
        }
      } catch (err) {
  ...
  const showtimeDays = useMemo(() => {
    ...
    let result = Object.keys(slotsByDate)
      .sort()
      .map((key) => {
    ...
    // Fallback: if no showtimeDays found, try building directly from movie.slots
    if ((!result || result.length === 0) && Array.isArray(movie.slots) && movie.slots.length) {
      const fb = {};
      for (const raw of movie.slots) {
        try {
          // prefer slotToISO but fall back to naive concat
          let iso = slotToISO(raw);
          if (!iso && raw.date && raw.time) iso = `${raw.date}T${String(raw.time).padStart(5, "0")}:00+05:30`;
          if (!iso) continue;
          const d = new Date(iso);
          if (Number.isNaN(d.getTime())) continue;
          const dateKey = formatDateKey(d, TZ);
          if (!fb[dateKey]) fb[dateKey] = [];
          const audi = (raw && raw.audi) || (raw && raw.auditorium) || movie.auditorium || "Audi 1";
          fb[dateKey].push({ iso, audi });
        } catch (e) {}
      }
      result = Object.keys(fb)
        .sort()
        .map((key) => {
          const [yy, mm, dd] = key.split("-").map(Number);
          const asDate = new Date(Date.UTC(yy, mm - 1, dd));
      ...
      console.debug("Fallback showtimeDays built from movie.slots:", result);
    }

    console.debug("Computed showtimeDays:", result, "rawSlots:", rawSlots);
    return result;
  }, [movie]);
